...
#Todo
  ...
  def handle_call({:play_turn, user, card}, _from, {game = %CardGame.Game{current_turn: user, current_trick: current_trick}, agent}) do
    ...
    if (is_card_valid(card, current_trick, user_cards)) do
      ...
      if len == 4 do
        ...
        {:reply, {user, card}, {%{new_game | user_cards: new_user_cards}, agent}}
      else
        next_user = Enum.at(game.users, rem(Enum.find_index(game.users, fn x -> x == user end) + 1, 4))
        {:reply, {user, card}, {%{game | current_turn: next_user, current_trick: new_trick, user_cards: new_user_cards}, agent}}
      end
    else
  ...
  def handle_call({:get_cards, user}, _from, {game, agent}) do
    user_cards = Map.get(game.user_cards, user)
  ...
  defp deal_user(user, [], game, cards) do
    %{game | user_cards: Map.put(game.user_cards, user, Enum.take(cards, 13))}
  end

  defp deal_user(user, [x | xs], game, cards) do
    new_game = %{game | user_cards: Map.put(game.user_cards, user, Enum.take(cards, 13))}
    rem_cards = Enum.drop(cards, 13)
    deal_user(x, xs, new_game, rem_cards)
  end

  def is_card_valid(card, trick, user_cards) do
    if Enum.member?(user_cards, card) do
      if Enum.empty?(trick) do
        true
      else
        {_, suit, _} = hd(Enum.reverse(trick)).card
        {_, card_suit, _} = card
        if suit == card_suit do
          true
        else
          Enum.find(user_cards, fn {_, c_suit, _} -> c_suit == suit end) == nil
        end
      end
    else
      false
  ...
  def remove_user_card(card, user, user_cards) do
    new_u_cards = List.delete(user_cards[user], card)
    Map.put(user_cards, user, new_u_cards)
  end

  def calculate_result(game = %CardGame.Game{state: @state_playing}) do
    current_trick_winner = find_trick_winner(game.current_trick, game.trump).user;
